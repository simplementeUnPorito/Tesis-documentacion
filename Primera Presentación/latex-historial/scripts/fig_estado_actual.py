...
for i, (key, name, dx, length, positions, weight) in enumerate(groups):
    ...
    ax.text(-2.2, y, name, ha="right", va="center", fontsize=9.5, color=INK)
ax.set_xlim(-3, max(g[4][-1] for g in groups) + 28)
ax.set_ylim(-0.65, len(groups) - 0.35)
ax.set_yticks([])
ax.set_xticks(np.arange(0, 55, 10))
ax.set_xlabel("Distancia a la fuente [m]", fontsize=9)
ax.set_title("Geometría persistida — apertura sintética con un geófono móvil",
...
fig, (a1, a2) = plt.subplots(1, 2, figsize=(9.8, 4.1), dpi=200,
...
for f, color in ((20, C_GREEN), (50, C_BLUE), (100, C_RED)):
    ...
    ax.text(budget + 8, 5.35, f"{budget:.0f} µs", fontsize=7.2, color=color)
...
ax.set_xlim(0, 750)
ax.set_ylim(0, 23)
ax.set_xlabel("Desfasaje temporal Δt [µs]", fontsize=9)
ax.set_ylabel("Error de fase Δφ [°]", fontsize=9)
...
print("MASW:", len(picks), "picks", m["inv_scalars"])
