...
la rigidez del esqueleto s\'olido. $V_S$, en cambio, est\'a controlada principalmente por
la rigidez de corte del esqueleto y es mucho menos sensible que $V_P$ al fluido
intersticial, aunque la saturaci\'on puede modificar densidad, esfuerzo efectivo y rigidez.
Por eso $V_S$ ---y no
...
el decaimiento geom\'etrico de amplitud: las ondas superficiales decaen aproximadamente
como $r^{-1/2}$ frente a $r^{-1}$ para ondas de cuerpo producidas por una fuente puntual
tridimensional (sus intensidades decaen como $r^{-1}$ y $r^{-2}$, respectivamente). La
combinaci\'on de ambos efectos hace
...
\caption{Esquema propio del principio de dispersi\'on. Las componentes de frecuencia
alta concentran su movimiento cerca de la superficie; las bajas penetran m\'as. La curva
$c(f)$ convierte cada componente en una longitud de onda $\lambda=c/f$, que determina la
escala de profundidad que puede informar el registro.}
\label{fig:fund-dispersion-visual}
\end{figure}
...
\emph{pipeline} desarrollado permite extraer grupos modales separados y dispone de un
\emph{backend} multimodal (Secci\'on~\ref{sec:arq-pipeline}). El resultado de campo
persistido que se analiza m\'as adelante usa solamente una curva etiquetada como modo~0,
sin identificaci\'on independiente como fundamental; capacidad y
resultado se distinguen expl\'icitamente.
