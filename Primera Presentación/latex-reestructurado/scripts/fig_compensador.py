...
fig, (ax, axr) = plt.subplots(
    2, 1, figsize=(6.30, 4.40), dpi=300, sharex=True,
...
fig, ax = plt.subplots(figsize=(6.10, 4.58), dpi=300)
...
estilo(ax)
ax.set_ylabel("Magnitud normalizada [dB]", fontsize=7.2, color=TINTA)
ax.set_xlim(1e-4, 1e3)
ax.set_ylim(-60, 6)
ax.axhline(-3, color="#b8b8b8", lw=1.0, ls=(0, (4, 3)), zorder=1)
ax.text(1.1e-4, -2.4, "-3 dB", fontsize=6.0, color=TENUE)
ax.axvline(5.44e-3, color=C_MFB, lw=0.9, ls=(0, (3, 3)), alpha=0.75)
ax.text(6.0e-3, -15, "5,44 mHz\n(polo ideal)", fontsize=6.0,
        color=C_MFB, ha="left", va="center")
ax.axvline(10, color="#b8b8b8", lw=1.0, ls=(0, (4, 3)), zorder=1)
ax.text(10.6, 3.2, "$f_n$ = 10 Hz", fontsize=6.0, color=TENUE)
ax.legend(frameon=False, fontsize=6.8, loc="lower right", labelcolor=TINTA)
ax.set_title("Efecto de la compensacion: extension de la banda hacia abajo",
             fontsize=8.0, color=TINTA, loc="left", pad=10)
fig.tight_layout()
